...
static double os_kinetic(double x1, double y1, double z1,
  ...
  double P[3],PA[3],PB[3],I1,I2,I3,I4,Ix,Iy,Iz;
  ...
  for (p1=0; p1<nbas1; p1++){
    ...
    for(p2=0;p2<nbas2; p2++){
      ...
      // l
      I1 = (l1==0 || l2==0) ? 0.0 : 
	0.5*l1*l2*x[(l1-1)*MAXAM+l2-1]*y[m1*MAXAM+m2]*z[n1*MAXAM+n2]*pre_fac;
      I2 = 2.0*a1*a2*x[(l1+1)*MAXAM+l2+1]*y[m1*MAXAM+m2]*z[n1*MAXAM+n2]*pre_fac;
      I3 = (l2==0) ? 0.0 :
	a1*l2*x[(l1+1)*MAXAM+l2-1]*y[m1*MAXAM+m2]*z[n1*MAXAM+n2]*pre_fac;
      I4 = (l1==0) ? 0.0 :
	a2*l1*x[(l1-1)*MAXAM+l2+1]*y[m1*MAXAM+m2]*z[n1*MAXAM+n2]*pre_fac;
      Ix = I1+I2-I3-I4;

      // m
      I1 = (m1==0 || m2==0) ? 0.0 : 
	0.5*m1*m2*x[l1*MAXAM+l2]*y[(m1-1)*MAXAM+m2-1]*z[n1*MAXAM+n2]*pre_fac;
      I2 = 2.0*a1*a2*x[l1*MAXAM+l2]*y[(m1+1)*MAXAM+m2+1]*z[n1*MAXAM+n2]*pre_fac;
      I3 = (m2==0) ? 0.0:
	a1*m2*x[l1*MAXAM+l2]*y[(m1+1)*MAXAM+m2-1]*z[n1*MAXAM+n2]*pre_fac;
      I4 = (m1==0) ? 0.0:
	a2*m1*x[l1*MAXAM+l2]*y[(m1-1)*MAXAM+m2+1]*z[n1*MAXAM+n2]*pre_fac;
      Iy = I1+I2-I3-I4;

      //n
      I1 = (n1==0||n2==0) ? 0.0:
	0.5*n1*n2*x[l1*MAXAM+l2]*y[m1*MAXAM+m2]*z[(n1-1)*MAXAM+n2-1]*pre_fac;
      I2 = 2.0*a1*a2*x[l1*MAXAM+l2]*y[m1*MAXAM+m2]*z[(n1+1)*MAXAM+n2+1]*pre_fac;
      I3 = (n2==0) ? 0.0:
	a1*n2*x[l1*MAXAM+l2]*y[m1*MAXAM+m2]*z[(n1+1)*MAXAM+n2-1]*pre_fac;
      I4 = (n1==0) ? 0.0:
	a2*n1*x[l1*MAXAM+l2]*y[m1*MAXAM+m2]*z[(n1-1)*MAXAM+n2+1]*pre_fac;
      Iz = I1+I2-I3-I4;
...
PYBIND11_MODULE(kinetic,m) {
  m.def("os_kinetic", &os_kinetic, "Kinetic");
}
